...
N = struct.N;
[para_grid, ~, size_grid, para_fieldNames]= GenGrid(para);
...
for k = 1:size(para_grid,1)
    ...
    elseif data_tensor_type == "slice-wise"
        ...
        parfor p = 1:length(NumFS)
            [ACC(p,k),NMI(p,k),STD_ACC(p,k),STD_NMI(p,k)] = Evaluation(data_ten,label,id,NumFS(p),NumClass,N,setting);
        end
    end
